...
setwd(targetFolder)
targets=list.files()
cur_color=strsplit(targets[index],"\\.")[[1]][1]
...
if(is.na(file.info(paste(targetFolder,targets[index],sep=""))$size)||file.info(paste(targetFolder,targets[index],sep=""))$size==0){

}else{


				
SEED <- read.table(paste(targetFolder,targets[index],sep=""),sep="\t") #*** Customize
SEED=as.matrix(SEED)
...
genes=SEED[,1]
kdFname <- paste( outputDir , flag,"_",cur_color , "_keydriver.xls" , sep = "" )
