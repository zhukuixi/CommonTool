...
drawCycleCraph<-function(inputFile,top=3,outFile,drawlimitModule){

	  ## inputFile: 3 column file
	  ## pathway p-value ModuleColor

	  ## top: pick top N significant pathway for each module

	  ## outFile: output png file address

	  ## drawlimitModule
	  ## If it is a array list, it means the graph only show module names in the list
	  ## If it is NULL, it means draw every module appear in the file
	  ...
	  uniqueModule = unique(input[,2])
	  data=c()
	  for(i in 1:length(uniqueModule)){
	    ind = which(input[,2]==uniqueModule[i]&input[,3]<0.05)
	    ind = ind[1:(min(top,length(ind)))]
	    data = rbind(data,input[ind,])
	  }
          
          if(!is.null(drawlimitModule)){
          	data = data[which(data[,2]%in%drawlimitModule),]
          }
  	
	  colnames(data)=c("individual","group","value")
	  
	  dealName<-function(name){
	  	if(grepl("target_",name)){
	  		return(strsplit(name,"target_")[[1]][2])
	  	}else{
	  		return(name)
	  	}
	  }
	  data[,2]=unlist(lapply(data[,2],dealName))
	  ############## draw   ##################
	 ...
	 #data$value = data$value * 100
	  ...
	  p = ggplot(data, aes(x=as.factor(id), y=value, fill=group)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
	    ...
	    geom_bar(aes(x=as.factor(id), y=value, fill=group), stat="identity", alpha=0.5) +
	    ylim(-40,maxValue) +      ### You can adjust this 2 numbers to change the graph. This two numbers indicates the lower and upper boudary of values showed in the cycle graph. The first negative value
	   ...
	   ggsave(outFile, height=12, width=12, units='in', dpi=600)
